...
public class Player extends GameEntity {
  ...
  public Player(float x, float y, World world) {
    super(x, y, Assets.atlas.createSprite("player"));
  }
  ...
  public void update(World world) {
    if (!world.demoMode) {
      if (Gdx.input.isKeyJustPressed(Input.Keys.Q) || health <= 0) {
        position.set(Constants.MEDBAY_COORDINATES[0], Constants.MEDBAY_COORDINATES[1]);
        confused = false;
        slowed = false;
      ...
      if (world.medbay.getRectangle().contains(position.x, position.y)) {
        health += Constants.AUBER_HEAL_RATE;
        health = Math.min(1f, health);
      }

      // Calculate velocity changes
      this.calculateNewVelocity(world);
      ...
      if (Gdx.input.isButtonPressed(Input.Buttons.LEFT) && teleporterRayCoordinates.isZero()) {
        world.auberTeleporterCharge = Math.min(world.auberTeleporterCharge
            + Constants.AUBER_CHARGE_RATE, 1f);
      } else {
        if (world.auberTeleporterCharge > 0.95f) {
          ...
          for (GameEntity entity : world.getEntities()) {
            ...
            if (entityDistance < Constants.NPC_EAR_STRENGTH && entity instanceof Npc) {
          ...
          }, Constants.AUBER_RAY_TIME);
        } else {
          world.auberTeleporterCharge = Math.max(world.auberTeleporterCharge
              - Constants.AUBER_CHARGE_RATE, 0f);
        }
      }
      if (Gdx.input.isKeyJustPressed(Input.Keys.E)) {
        // Interact with an object
        RectangleMapObject nearbyObject = getNearbyObjects(Assets.map);
  ...
  void calculateNewVelocity(World world) {
