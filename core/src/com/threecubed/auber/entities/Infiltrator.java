...
public class Infiltrator extends Npc {
  ...
  public void handleDestinationReached(World world) {
    ...
    if (oldState != States.FLEEING) {
      if (!playerNearby(world)
          ...
          < Constants.SYSTEM_SABOTAGE_CHANCE) {
  ...
  public void handleTeleporterShot(final World world) {
    if (state == States.ATTACKING_SYSTEM) {
      RectangleMapObject system = getNearbyObjects(Assets.map);
      if (system != null) {
    ...
    if (!exposed) {
      ...
      sprite = Assets.atlas.createSprite("infiltrator");
      ...
      }, Constants.INFILTRATOR_FIRING_INTERVAL, Constants.INFILTRATOR_FIRING_INTERVAL);
    } else {
      position.x = Utils.randomFloatInRange(world.randomNumberGenerator,
          Constants.BRIG_BOUNDS[0][0], Constants.BRIG_BOUNDS[1][0]);
      position.y = Utils.randomFloatInRange(world.randomNumberGenerator,
          Constants.BRIG_BOUNDS[0][1], Constants.BRIG_BOUNDS[1][1]);
      aiEnabled = false;    
    }
  }
  ...
  private void attackNearbySystem(final World world) {
    ...
    final RectangleMapObject system = getNearbyObjects(Assets.map);
    if (system != null) {
  ...
  private boolean playerNearby(World world) {
    ...
    Circle infiltratorSight = new Circle(position, Constants.INFILTRATOR_SIGHT_RANGE);
  ...
  private void fireProjectileAtPlayer(World world) {
    ...
    projectileVelocity.setLength(Constants.INFILTRATOR_PROJECTILE_SPEED);
